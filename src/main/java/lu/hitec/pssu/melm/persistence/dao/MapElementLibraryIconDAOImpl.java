...
public class MapElementLibraryIconDAOImpl implements MapElementLibraryIconDAO {
	...
	@Override
	@Transactional
	public MapElementLibraryIcon addIconToLibrary(final MapElementLibrary library, final MapElementIcon icon, final int indexOfIconInLibrary, final String iconNameInLibrary,
	...
	public boolean checkIconInLibrary(final MapElementIcon icon) {
		final TypedQuery<MapElementLibraryIcon> query = em.createQuery("SELECT meli FROM MapElementLibraryIcon meli WHERE meli.icon = :icon", MapElementLibraryIcon.class);
	...
	public boolean checkIconInLibrary(final MapElementLibrary library, final MapElementIcon icon) {
		final TypedQuery<MapElementLibraryIcon> query = em.createQuery("SELECT meli FROM MapElementLibraryIcon meli WHERE meli.library = :library AND meli.icon = :icon", MapElementLibraryIcon.class);
	...
	public List<MapElementLibraryIcon> getIconsInLibrary(@Nonnull final MapElementLibrary library) {
		...
		final TypedQuery<MapElementLibraryIcon> query = em.createQuery("SELECT meli FROM MapElementLibraryIcon meli WHERE meli.library = :library ORDER BY meli.indexOfIconInLibrary",
	...
	public MapElementLibraryIcon getPreviousLibraryIcon(final MapElementLibraryIcon libraryIcon) {
		...
		final TypedQuery<MapElementLibraryIcon> query = em.createQuery(
				"SELECT meli FROM MapElementLibraryIcon meli WHERE meli.indexOfIconInLibrary < :indexOfIconInLibrary ORDER BY meli.indexOfIconInLibrary DESC", MapElementLibraryIcon.class).setMaxResults(1);
	...
	public MapElementLibraryIcon getNextLibraryIcon(final MapElementLibraryIcon libraryIcon) {
		...
		assert libraryIcon != null : "Library icon is null";
		final TypedQuery<MapElementLibraryIcon> query = em.createQuery(
				"SELECT meli FROM MapElementLibraryIcon meli WHERE meli.indexOfIconInLibrary > :indexOfIconInLibrary ORDER BY meli.indexOfIconInLibrary ASC", MapElementLibraryIcon.class).setMaxResults(1);
		query.setParameter("indexOfIconInLibrary", libraryIcon.getIndexOfIconInLibrary());
		try {
			result = query.getSingleResult();
		} catch (final NoResultException e) {
	...
	@Override
	@Transactional
	public void updateLibraryIcon(final long id, @Nonnull final MapElementIcon icon, final int indexOfIconInLibrary, @Nonnull final String iconNameInLibrary,
			@Nonnull final String iconDescriptionInLibrary) {
